...
class DatabaseConnection extends Connection
{
    ...
    public function pop()
    {
        ...
        if ($next)
        {
            $this->container['db']->update($this->config['table'], [
    ...
    {
        $this->container['db']->statement('update ' . $this->config['table'] . ' set state=?, pid=? where queue_name=? and state=? and pid is null order by id limit ' . $limit, [
            BackgroundEvent :: STATE_PROCESS,
            posix_getpid(),
            $this->queueName,
            BackgroundEvent :: STATE_NEW
        ]);
        return $this->container['db']->select('select * from ' . $this->config['table'] . ' where queue_name=? and state=? and pid=?', [
            $this->queueName,
            BackgroundEvent :: STATE_PROCESS,
            posix_getpid()
        ]);
    ...
    {
        $ids = (array) $ids;
        $this->container['db']->statement('update ' . $this->config['table'] . ' set state=? where id in (?)', [
            BackgroundEvent :: STATE_DONE,
            $ids
        ]);
        //$this->container['db']->statement('delete from ' . $this->config['table'] . ' where id in (?)', [$ids]);
    }

    public function fail($ids, $error)
    {
        $ids = (array) $ids;
        $this->container['db']->statement('update ' . $this->config['table'] . ' set state=?, error=? where id in (?)', [
